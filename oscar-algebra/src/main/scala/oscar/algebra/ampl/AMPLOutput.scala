...
class AMPLOutput[O <: AnyType, C <: AnyType](implicit val model: Model[O, C]) {
...
def stringOfConstant(v: Any) = {
  ...
  def stringOfVar(v: Var): String = {
    v match{
      case vr: Var1Ref[_] if vr.v.range.values.head.isInstanceOf[Int] => v.toString
      case vr: Var1Ref[_] => s"""${vr.v.name}["${vr.index}"]"""
      case vr: Var2Ref[_,_] =>
  ...
  def stringOf[T <: AnyType](term: Term[T]): String = {
    term match{
      case v: Var => stringOfVar(v)
      case s: SumLoop[_,T] => s"""sum {${s.aLoop.iterator(AMPLOutput.defaultIndices)}}(${s.aLoop.body(AMPLOutput.defaultIndices)})"""
      case p: Param1Ref[_] => s"${p.param.name}[${stringOfConstant(p.index)}]"
    }
  }

  def stringOf[T <: AnyType](p: Prod[T], first: Boolean): String = {
    if (p.coef.d == 0) ""
  ...
  def stringOf[T <: AnyType](e: Expression[T]): String = {
    {
      e.terms match {
  ...
  def stringOf[T <: AnyType](e: Equation[T]): String = {
  ...
  def solve: ComputationResult[SolutionBuffer[O, C]] = {
    ...
    output.head match {
      case successRegex(objValue) =>
        ...
        val solution = new SolutionBuffer[O, C]()
