...
import oscar.algebra.ampl.{AMPLOutput, Optimality}
...
class AMPLTests extends FunSuite with ShouldMatchers {
  ...
  test("Model") {
    ...
    implicit val model = new Model[Linear, Linear]()

    val Set = Indices("VS", 0 until 10)

    val vs = Var1(Set, "X", Some(0), Some(20))

    model.subjectTo(for (i <- Set) yield "E1" ||: Const(10) >= vs(i))

    model.subjectTo("C1" ||: Const(2) <= vs(2))
    ...
    out.write
    out.solve match {
      case Optimality(solution: SolutionBuffer[Linear,Linear]) => solution(vs.get(2)) shouldBe 2
    }
  ...
  test("Var2"){
    ...
    val vars = Var2(SetA,SetB, "X", Some(10))

    model.subjectTo("C" ||: (for(a <- SetA) yield s"C_$a" ||: vars(a)(2) <= vars(a)(3) - 10))
    ...
    out.write
    out.solve match {
      case Optimality(solution: SolutionBuffer[Linear,Linear]) => solution(vars.get(0)(3)) shouldBe 20
    }
  }
  ...
  test{"Symbolic Sums"}{
    ...
    model.subjectTo("E" ||: Const(2) >= new SumLoop[String,Linear](for(a <- SetA) yield vars(a).toExpression).toExpression )
  ...
  test("Parameters"){
    ...
    val param = new Param1("P",SetA,Map("a" -> 3, "b" -> 4), None)

    model.subjectTo("E" ||: Const(2) >= new SumLoop[String,Linear](for(a <- SetA) yield param(a).toExpression).toExpression )
    val out = new AMPLOutput
