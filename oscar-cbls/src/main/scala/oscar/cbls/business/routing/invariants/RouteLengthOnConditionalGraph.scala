...
class RouteLengthOnConditionalGraph(routes:ChangingSeqValue,
  ...
  def computeDistance(fromNode:Int,toNode:Int): AStarInfo ={
    ...
    val result = aStarEngine.search(graph.nodes(fromNode),graph.nodes(toNode), isConditionalEdgeOpen)

    val key = result match{
      case d:Distance =>
        val key:ValueWiseKey = registerDynamicValueWiseDependency(openConditions)
        for(c <- d.requiredConditions){
          key.addToKey(c)
        }
        for(c <- d.unlockingConditions){
          key.addToKey(c)
        }
        key
      case n:NeverConnected =>
        null
      case n:NotConnected =>
        val key:ValueWiseKey = registerDynamicValueWiseDependency(openConditions)
        for(c <- n.unlockingConditions){
          key.addToKey(c)
        }
        key
    }

    new AStarInfo(minNode, maxNode, result, allocAstarID, key)
  }
