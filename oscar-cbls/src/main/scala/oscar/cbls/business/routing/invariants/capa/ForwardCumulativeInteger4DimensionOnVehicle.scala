...
class ForwardCumulativeInteger4DimensionOnVehicle(routes:ChangingSeqValue,
  ...
  def check(s:IntSequence){
    ...
    for(node <- 0 until n){
      require(nodeToContent(node) == (content1AtNode(node).newValue,content2AtNode(node).newValue,content3AtNode(node).newValue,content4AtNode(node).newValue),
    ...
    for(vehicle <- 0 until v){
      require((content1AtEnd(vehicle).newValue,content2AtEnd(vehicle).newValue,content3AtEnd(vehicle).newValue,content4AtEnd(vehicle).newValue) == vehicleToContentAtEnd(vehicle),
