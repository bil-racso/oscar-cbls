...
object ForwardCumulativeInteger5DimensionOnVehicle {
        ...
        def apply(routes:ChangingSeqValue,
                  n:Int,
                  v:Int,
                  op:(Int,Int,Int,Int,Int,Int,Int)=>(Int,Int,Int,Int,Int),
                ...
                val content5AtNode = Array.tabulate(n)((node: Int) => CBLSIntVar(routes.model, 0, Domain.coupleToDomain(minContent,maxContent).union(default2ForUnroutedNodes), contentName + "5 at node "+node))
                ...
                val content5AtEnd = Array.tabulate(v)((vehicle: Int) => CBLSIntVar(routes.model, 0, Domain.coupleToDomain(minContent,maxContent), contentName + "5 at end of route " + vehicle))
...
class ForwardCumulativeInteger5DimensionOnVehicle(routes:ChangingSeqValue,
                                                  ...
                                                  defaultVehicleContent5ForUnroutedNodes:Int)
extends AbstractForwardCumulativeDimensionOnVehicle(routes,n,v) with IntNotificationTarget{
        ...
        content4AtNode(vehicle) := newValue5
        ...
        //(fromNode,toNode,content1AtFromNode,content2AtFromNode)=> (content1AtToNode,content2AtToNode)
        def op5(fromNode:Int,toNode:Int,content:(Int,Int,Int,Int,Int)) = op(fromNode,toNode,content._1,content._2,content._3,content._4,content._5)
        ...
        for(node <- 0 until n){
        require(nodeToContent(node) == (content1AtNode(node).newValue,content2AtNode(node).newValue,content3AtNode(node).newValue,content4AtNode(node).newValue,content5AtNode(node).newValue),
        ...
        for(vehicle <- 0 until v){
        require((content1AtEnd(vehicle).newValue,content2AtEnd(vehicle).newValue,content3AtEnd(vehicle).newValue,content4AtEnd(vehicle).newValue,content5AtEnd(vehicle).newValue) == vehicleToContentAtEnd(vehicle),
