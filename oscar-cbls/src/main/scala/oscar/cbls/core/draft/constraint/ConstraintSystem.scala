...
case class ConstraintSystem(model:Store) extends Constraint with Objective{
  ...
  /**Method used to post a constraint in the constraint system. (synonym of add)
    ...
    */
  def add(c:Constraint,weight:ChangingIntValue = null){
    constraints = (c,weight) :: constraints
  }
  ...
  private def computeLocalViolationOnConstrainedVariable:(QList[ChangingValue],NodeDictionary[QList[ChangingIntValue]]) = {
    ...
    for((constraint,weight) <- constraints){
      ...
      for((constrainedVar,varViolation) <- constraint.constrainedVariablesAndViolation) {
        ...
        val newViolation = if(weightIsNull) varViolation else Prod2(varViolation,weight)
        dicoForLocalViolation(constrainedVar) = QList(newViolation,alreadyKnown)
      }
    }
...
/*
  private def computeViolationOnDecisionVariables(){
    ...
    //1: compute the violation on each constrained variable of each constraint (store it in a global storage array)


    //2: get sources for each constrained variable we know that they all have different violation so thi proces must be repeated
  ...
  //SourceFinderAlgo(propagationStructure:PropagationStructure)
...
*/
  ...
  def violatedConstraints:List[Constraint] =
    constraints.filter(p => (p._2 == null || p._2.value !=0) && !p._1.isTrue).map(p => p._1)
