...
class SourceFinderAlgo(propagationStructure:PropagationStructure) {
  ...
  val marking = propagationStructure.buildNodeStorage[Boolean]
  ...
  def getSources(pe:PropagationElement):QList[PropagationElement] = {
    unMarkAll()
    var toExplore: QList[PropagationElement] = QList(pe)
