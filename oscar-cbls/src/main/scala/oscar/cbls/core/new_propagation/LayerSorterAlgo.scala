...
/**
  * This class performs the sorting of propagation elements that are not
  * Strongly connected components
  *
  * @param propagationElementsNotInSCC a list of propagation elements not SCC
  */
class LayerSorterAlgo(propagationElementsNotInSCC:QList[PropagationElement]) {
  ...
  def sortNodesByLayer(): (Array[Int],Array[QList[PropagationElement]]) = {
    ...
    //finished iteration
    require(count == nbPENotInSCC, s"Internal bug: count=$count != nbPENotInScc=$nbPENotInSCC")
  ...
  private def peInSCC(pe:PropagationElement):Boolean = {
    val s = pe.schedulingHandler
    s != null && s.isSCC
  }

  private def nbPENotInSCC(pes:Iterable[PropagationElement]):Int = pes.count(!peInSCC(_))

  /**
    * Sets the layer of the PE to the number of other PE that must be scheduled
    * before this one.
    * For scc, set it to the number of element that are referenced from other
    * components or from the global SCC
    *
    * @param pe the propagation element
