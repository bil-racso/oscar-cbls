...
case class DisjunctiveWithTransitions(start: Array[IntValue],
                                      ...
                                      marginMatrix:Array[Array[Long]]) extends Invariant with Constraint with IntNotificationTarget{
  ...
  def updateTask(taskID:Long,oldStart:Long,newStart:Long,oldEnd:Long,newEnd:Long){
    ...
    for(otherTaskID <- nonZeroTasks if taskID != otherTaskID){
      ...
      val oldOverlap = math.max(0L,math.min(otherEnd + otherMargin,oldEnd + curMargin)-math.max(otherStart, oldStart))
      val newOverlap = math.max(0L,math.min(otherEnd + otherMargin,newEnd + curMargin)-math.max(otherStart, newStart))
      val deltaViolation = newOverlap - oldOverlap
