...
object OBSSolver extends CompetitionApp with App {
  ...
  override def runSolver(conf: CompetitionConf): Unit = {
    ...
    if (parsingResult.isDefined) {
      ...
      var searchStrat = if (auxiliaryVars.isEmpty) {
        ConflictOrderingSearch(new BinaryRandomizedNogoodBranching(decisionVars, decOBS.getDeltaO, decBIVS.selectValue))(solver)
      }
      else {
        val h1 = new BinaryRandomizedNogoodBranching(decisionVars, decOBS.getDeltaO, decBIVS.selectValue)
        val h2 = new BinaryRandomizedNogoodBranching(auxiliaryVars, auxOBS.getDeltaO, auxBIVS.selectValue)
      ...
      searchStrat = if (auxiliaryVars.isEmpty) {
        new BinaryRandomizedNogoodBranching(decisionVars, decOBS.getDeltaO, decSBPS.selectValue)
      }
      else {
        val h1 = new BinaryRandomizedNogoodBranching(decisionVars, decOBS.getDeltaO, decSBPS.selectValue)
        val h2 = new BinaryRandomizedNogoodBranching(auxiliaryVars, auxOBS.getDeltaO, auxSBPS.selectValue)
        h1 ++ h2
      }
      ...
      searchStrat = if(auxiliaryVars.isEmpty) {
        ConflictOrderingSearch(new HeuristicNogoodBranching(decisionVars, decOBS.getDeltaO, i => decisionVars(i).randomValue(rand)))(solver)
      }
      else {
        val h1 = new HeuristicNogoodBranching(decisionVars, decOBS.getDeltaO, i => decisionVars(i).randomValue(rand))

        val h2 = new HeuristicNogoodBranching(auxiliaryVars, auxOBS.getDeltaO, i => auxiliaryVars(i).randomValue(rand))
        ConflictOrderingSearch(h1)(solver) ++ ConflictOrderingSearch(h2)(solver)
      }
      noGoodSearch.start(searchStrat, stopCondition)
