...
import oscar.algo.branchings.OptionalDecisionSearch
import oscar.cp._
...
/**
  ...
  * The objective is to maximize the number of patients transported. If a patient is transported to an appointment,
  * they must be brought back after the end of the appointment (but not necessarily by the same vehicle).
  * The patients can only be transported by compatible vehicles.
  *
...
object PatientTransportProblem extends CPModel with App {
  ...
  val nPatient = 8
  val nVehicle = 3 //Two real and one dummy (0) vehicle

  val patientRdv = Array(60, 60, 120, 150, 240, 300, 300, 300)
  val patientDur = Array(20, 120, 40, 40, 60, 20, 20, 60)
  val patientSrv = Array(2, 3, 2, 2, 5, 2, 2, 2)
  val patientRdvEnd = patientRdv.indices.map(i => patientRdv(i) + patientDur(i))
  val patientLoad = Array(1, 2, 1, 1, 1, 2, 1, 1)
  val patientCategory = Array(0, 1, 0, 0, 1, 0, 0, 0)

  val vehicleCap = Array(patientLoad.sum, 6, 4)
  val vehicleDepot = Array(18, 7, 8)
  val vehicleCompatibility = Array(
    Set(0, 1),
    Set(0),
    Set(0, 1)
  )
  ...
  val locationMatrix = Array(
    Array(0, 2, 6, 3, 3, 11, 2, 11, 9, 9, 0, 12, 9, 13, 9, 11, 10, 11, 0),
    Array(2, 0, 7, 1, 0, 11, 2, 10, 8, 11, 2, 13, 11, 15, 6, 13, 9, 13, 0),
    Array(6, 7, 0, 8, 8, 9, 7, 11, 10, 4, 6, 10, 5, 8, 13, 6, 13, 6, 0),
    Array(3, 1, 8, 0, 1, 11, 3, 9, 7, 12, 3, 13, 12, 15, 5, 13, 8, 14, 0),
    Array(3, 0, 8, 1, 0, 11, 2, 10, 7, 11, 2, 12, 11, 15, 6, 13, 8, 13, 0),
    Array(11, 11, 9, 11, 11, 0, 12, 5, 6, 12, 11, 2, 13, 7, 14, 7, 9, 9, 0),
    Array(2, 2, 7, 3, 2, 12, 0, 12, 10, 10, 2, 14, 10, 15, 8, 13, 11, 13, 0),
    Array(11, 10, 11, 9, 10, 5, 12, 0, 2, 16, 11, 6, 17, 13, 10, 12, 4, 14, 0),
    Array(9, 8, 10, 7, 7, 6, 10, 2, 0, 14, 8, 7, 15, 13, 8, 12, 3, 13, 0),
    Array(9, 11, 4, 12, 11, 12, 10, 16, 14, 0, 9, 14, 1, 9, 17, 7, 17, 6, 0),
    Array(0, 2, 6, 3, 2, 11, 2, 11, 8, 9, 0, 12, 9, 13, 8, 11, 10, 11, 0),
    Array(12, 13, 10, 13, 12, 2, 14, 6, 7, 14, 12, 0, 15, 8, 15, 7, 10, 10, 0),
    Array(9, 11, 5, 12, 11, 13, 10, 17, 15, 1, 9, 15, 0, 10, 17, 9, 18, 7, 0),
    Array(13, 15, 8, 15, 15, 7, 15, 13, 13, 9, 13, 8, 10, 0, 20, 2, 16, 4, 0),
    Array(9, 6, 13, 5, 6, 14, 8, 10, 8, 17, 8, 15, 17, 20, 0, 18, 7, 19, 0),
    Array(11, 13, 6, 13, 13, 7, 13, 12, 12, 7, 11, 7, 9, 2, 18, 0, 14, 3, 0),
    Array(10, 9, 13, 8, 8, 9, 11, 4, 3, 17, 10, 10, 18, 16, 7, 14, 0, 16, 0),
    Array(11, 13, 6, 14, 13, 9, 13, 14, 13, 6, 11, 10, 7, 4, 19, 3, 16, 0, 0),
  ...
  val vehicleForward: Array[CPIntVar] = patientForward.map(p => CPIntVar(
    Array(0) ++ (1 until nVehicle).filter(v => vehicleCompatibility(v).contains(patientCategory(p)))
  ))
  ...
  for(i <- allPatient.indices) {
    ...
    add(visited(p) ==> (allStart(i) ?>= locationMatrix(i)(vehicleDepot(allVehicle(i)))))
    add(visited(p) ==> ((allEnd(i) + patientDur(p)) ?<= (-locationMatrix(i)(vehicleDepot(allVehicle(i))) + horizon)))
  ...
  val decVars = (for (p <- 0 until nPatient) yield {
    ...
    CPOptionalDecision(decisionVar, optionalVars, binaryFirstFail(optionalVars, _.min))
  }).toArray

  search(OptionalDecisionSearch(
    decVars,
    conflictOrderingSearch(decVars.map(_.decisionVar), (i) => -patientRdv(i), _ => 1)
  ))
