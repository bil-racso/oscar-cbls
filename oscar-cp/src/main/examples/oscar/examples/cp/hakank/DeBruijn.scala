...
object DeBruijn {
  ...
  def main(args: Array[String]) {
    ...
    val x        = Array.fill(m)(CPIntVar(0 to pow(base, n).toInt - 1)(cp))
    val binary   = Array.fill(m,n)(CPIntVar(0 to base-1)(cp))
    val bin_code = Array.fill(m)(CPIntVar(0 to base-1)(cp))
    val gccv     = Array.tabulate(base)(i => (i,CPIntVar(0 to m)(cp)))
    ...
    cp.solve subjectTo {
      ...
      // channeling x <-> binary
      for (i <- 0 until m) {
         val t = Array.tabulate(n)(j=> CPIntVar(0 to base-1)(cp))
