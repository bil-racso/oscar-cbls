...
public class GCCBinPacking extends Constraint {
	...
	@Override
	public CPOutcome setup(CPPropagStrength st) {
	...
	private boolean pruneBounds() {
		...
		for (int v = 0; v < o.length; v++) {
			...
			if (m != M) {
				...
				if (o[v].updateMin(up[v + minValInit - minVal]) == CPOutcome.Failure) {
					return false;
				}
				up[v + minValInit - minVal] = M;
			}
		...
		for (int v = 0; v < o.length; v++) {
			...
			if (m != M) {
				...
				if (o[v].updateMax(low[v + minValInit - minVal]) == CPOutcome.Failure) {
					return false;
				}
			...
			if (currLoad < l[v - minVal].min()
					|| o[v - minVal].updateMin(currCard) == CPOutcome.Failure) {
				return false;
			}
			...
			if (l[v - minVal].updateMax(currLoad) == CPOutcome.Failure) {
				return false;
			}
			...
			while (k >= 0 && currLoad + w[k] <= l[v - minVal].max()
				...
				if (!alreadyPacked[k] && x[k].hasValue(v)  && (varMatch[k] == v || findResidualPathValue(v, k, false))) {
					...
					if (currCard == o[v - minVal].min()) {
						if (l[v - minVal].updateMin(currLoad) == CPOutcome.Failure) {
							return false;
						}
					}
				}
			...
			if (o[v - minVal].updateMax(currCard) == CPOutcome.Failure) {
				return false;
			}
