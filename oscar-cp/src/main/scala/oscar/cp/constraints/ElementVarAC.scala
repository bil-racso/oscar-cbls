...
class ElementVarAC(y: Array[CPIntVar], x: CPIntVar, z: CPIntVar) extends Constraint(x.store, "ACElementVar") {
  ...
  // Remove v from y(i)
  final override def valRemoveIdx(cpvar: CPIntVar, i: Int, v: Int): CPOutcome = {
  ...
  final override def valRemove(cpvar: CPIntVar, v: Int): CPOutcome = {
    if (cpvar == x) removeFromX(v)
    else removeFromZ(v)
  }

  // Reduces the number of supports of the value v
  @inline private def reduceSupports(value: Int): CPOutcome = {
  ...
  @inline private def removeFromX(id: Int): CPOutcome = {
    if (x.isBound) bindX()
    else {
