...
import oscar.algo.search.{Alternative, Branching, BranchingUtils, DiscrepancyBranching, branch, isInconsistent, noAlternative}
...
trait Branchings extends BranchingUtils {
  ...
  /**
    * @author Pierre Schaus pschaus@gmail.com
    */
  class AuxCOS(solver: CPSolver, variables: Array[IntVarLike], varHeuristic: (Int) => Int, valHeuristic: (Int) => Int) extends Branching {

    require(variables.length > 0)

    private[this] val context = variables(0).context
    private[this] val nDecisions = Array.ofDim[Long](variables.length)
    private[this] val nBacktracks = Array.ofDim[Long](variables.length)

    solver.onSolution{
      println("number of Decisions: " + getDecisions())
      println("number of backtracks: " + getBacktracks())
    }
    ...
    def getDecisions():Long = {
      nDecisions.sum
    }

    def getBacktracks(): Long = {
      nBacktracks.sum
    }
