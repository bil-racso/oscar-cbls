...
class UnaryDP2(starts: Array[CPIntVar], durations: Array[CPIntVar], ends: Array[CPIntVar], resources: Array[CPIntVar], id: Int)(implicit store: CPStore)
extends Constraint(store, "UnaryDetectablePrecedences2") {
  val lr = new UnaryDP2LR(starts, durations, ends, resources, id) 
  val rl = new UnaryDP2LR(ends map(-_), durations, starts map(-_), resources, id)
...
class UnaryDP2LR(starts: Array[CPIntVar], durations: Array[CPIntVar], ends: Array[CPIntVar], resources: Array[CPIntVar], id: Int)(implicit store: CPStore)
extends UnaryTemplate(starts, durations, ends, resources, id, "UnaryDetectablePrecedencesLR")(store)
{
  ...
  override def propagate(): CPOutcome = {
    ...
    // if (debug) println(s"UnaryDP2: considering $nToConsider over $nTasks tasks, making a tree of ${2*nodes} nodes.")
    ...
    while (nPruningEvents > 0) {
      ...
      if (!required(i)) {
        ...
        while (envelopeOpt(1) > smax(i)) {
          ...
          // if (debug) println(s"UnaryDP2 removing $b")
    ...
    // println(s"UnaryDP2 @${store.magic}for $id: used $count over $p tasks in the tree")
    removeExtremal()
...
object UnaryDP2 {
  def apply(starts: Array[CPIntVar], durations: Array[CPIntVar], ends: Array[CPIntVar], resources: Array[CPIntVar], id: Int)(implicit store: CPStore) = 
    new UnaryDP2(starts, durations, ends, resources, id) 
