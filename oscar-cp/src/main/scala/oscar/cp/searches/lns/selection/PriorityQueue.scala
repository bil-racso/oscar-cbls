...
class PriorityQueue[A](implicit val ord: Ordering[A])
  extends AbstractIterable[A]
    ...
    with Growable[A]
    with mutable.Builder[A, PriorityQueue[A]]
    with Serializable
    with scala.Cloneable {

  import ord._

  private class ResizableArrayAccess[A] extends AbstractSeq[A] with mutable.ResizableArray[A] with Serializable {
    def p_size0 = size0
  ...
  /** Returns a regular queue containing the same elements.
    ...
    */
  def toQueue: mutable.Queue[A] = new mutable.Queue[A] ++= this.iterator
...
object Test extends App {
  val maxHeapLeft = PriorityQueue[Double]()
