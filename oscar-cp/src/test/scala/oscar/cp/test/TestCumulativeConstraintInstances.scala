...
/*
 *   Generates instances for cumulative constraints,
 *   filter them with given cumulative stack,
 *   and check that filtered values are actually never part of a solution.
 *   This allows to catch situations where a cumulative propagator has removed a value that it should not have. 
 */
/*
abstract class TestCumulativeConstraintInstances(val cumulativeName: String, val nTests: Int = 1000, val minDuration: Int = 0, val k: Int = 5) extends TestSuite {
  type Ints = Array[Int]
