...
import oscar.cp.searches.lns.operators.ClosenessStore
...
object RelaxationFunctions {
  ...
  /**
    ...
    */
  def randomRelax(model: CPModel, vars: Iterable[IntVar], currentSol: CPIntSol, k: Int): Unit = {
...
//    println("relaxing " + k + " variables")
    ...
    while(nToFreeze > 0){
      ...
      val x = varArray(i)
      model.add(varSeq(x) === currentSol.values(x))
...
//    println("relaxation done, " + (vars.size - k) + " vars frozen")
  ...
  /**
    ...
    * @param k The number of variables to relax (must be >= 0 and < vars.size)
    */
  def successiveRelax(model: CPModel, vars: Iterable[IntVar], currentSol: CPIntSol, k: Int): Unit = {
//    println("relaxing " + k + " variables")
    val varSeq = vars.toSeq
    ...
    if(start + k > varSeq.length)
      varSeq.indices.filter(i => i < start && i >= (start+k)-varSeq.length).foreach(
        i => {
          model.add(varSeq(i) === currentSol.values(i))
...
//    println("relaxation done, " + (varSeq.length - k) + " vars frozen")
  ...
  /**
    ...
    */
  def propagationGuidedRelax(model: CPModel, vars: Iterable[IntVar], currentSol: CPIntSol, closeness:ClosenessStore, s: Double): Unit = {
//    println("relaxing to size " + s)
    ...
    while (size > s) {
      ...
      model.add(varSeq(next) === currentSol.values(next)) //Freezing var
...
//    println("relaxation done, " + (varSeq.length - boundStart) + " vars frozen")
  ...
  /**
    ...
    * @param s The estimated size of the neighbourhood to attain.
    */
  def reversedPropagationGuidedRelax(model: CPModel, vars: Iterable[IntVar], currentSol: CPIntSol, closeness: ClosenessStore, s: Double): Unit = {
//    println("relaxing to size " + s)
    val varSeq = vars.toSeq
    val varArray = varSeq.indices.toArray //map to real index of variables
    ...
    for(i <- (0 until relaxStart).map(x => varArray(x))){
      model.add(varSeq(i) === currentSol.values(i)) //Freezing var
      if (!varSeq(i).isBound) throw Inconsistency
    }
