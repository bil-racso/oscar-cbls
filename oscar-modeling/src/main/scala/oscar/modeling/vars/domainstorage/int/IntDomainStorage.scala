package oscar.modeling.vars.domainstorage.int
...
object IntDomainStorage {
   ...
   * @param s
   */
  def apply(s: Set[Int], name: String) = new AdaptableIntDomainStorage(new SetDomainStorage(SortedSet(s.toList: _*), name))
  def apply(s: Set[Int]) = new AdaptableIntDomainStorage(new SetDomainStorage(SortedSet(s.toList: _*)))
  /**
   * Create a new IntDomainStorage from a Set of values
   * @param s
   */
  def apply(s: SortedSet[Int], name: String) = new AdaptableIntDomainStorage(new SetDomainStorage(s, name))
   ...
   * @param v
   */
  def apply(v: Int, name: String) = new AdaptableIntDomainStorage(new SingletonDomainStorage(v, name))
   ...
   * @param max
   */
  def apply(min: Int, max: Int, name: String) = new AdaptableIntDomainStorage(new IntervalDomainStorage(min, max, name))
