...
import oscar.visualfx.{Grid, Queens}
 ...
 */
object NQueensStandardNeighbourhoods extends CBLSModel with JFXApp with StopWatch{
  ...
  val _init = Random.shuffle(range.toList)
  val queens = Array.tabulate(N)((q:Int) => CBLSIntVar(_init(q), 0 until N, "queen" + q))
  val visualQueens = Array.tabulate(N)((q:Int) => new Queens(grid.grid, queens(q),(q,_init(q)),q,queenHeight*4/5,queenHeight))
  println(visualQueens.toString)
