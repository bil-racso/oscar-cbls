...
class VRPMaxDemo extends StopWatch{
  ...
  def optim(window: RoutingWindow, plot:Plot, symmetricDistanceMatrix1:Array[Array[Int]], nodesPositions: Array[(Int,Int)]): Unit = {
    ...
    )) exhaust profile(vlsn(40) maxMoves 1)).afterMoveOnMove((m:Move) => {
      window.update(Array.tabulate(v)(i => myVRP.getRouteOfVehicle(i)))
