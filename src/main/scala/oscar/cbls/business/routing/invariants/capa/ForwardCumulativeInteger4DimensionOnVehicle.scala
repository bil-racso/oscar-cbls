  ...
  def check(c : Checker,s:IntSequence): Unit ={
    ...
    for(node <- 0 until n){
      c.check(nodeToContent(node) == (content1AtNode(node).newValue,content2AtNode(node).newValue,content3AtNode(node).newValue,content4AtNode(node).newValue),
        Some(s"Vehicle content at node($node) at pos : ${s.positionsOfValue(node)} := ${(content1AtNode(node).newValue,content2AtNode(node).newValue,content3AtNode(node).newValue,content4AtNode(node).newValue)} should be :=${nodeToContent(node)} routes:${s.mkString(",")}"))
    }
    ...
    for(vehicle <- 0 until v){
      c.check((content1AtEnd(vehicle).newValue,content2AtEnd(vehicle).newValue,content3AtEnd(vehicle).newValue,content4AtEnd(vehicle).newValue) == vehicleToContentAtEnd(vehicle),
        Some(s"Error on vehicle content at end vehicle:$vehicle contentAtEnd(vehicle).newValue:${(content1AtEnd(vehicle).newValue,content2AtEnd(vehicle).newValue,content3AtEnd(vehicle).newValue,content4AtEnd(vehicle).newValue)} should be:${vehicleToContentAtEnd(vehicle)}"))
    }
  }
}
